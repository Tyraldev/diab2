...
const SK = "diabete-v5";
const VERSION = "v2.3 - Dexcom Live";
...
async function aiGlucidesPhoto(photoB64, desc, apiKey) {
  ...
  try {
    res = await fetch("https://api.anthropic.com/v1/messages", {
      method:"POST", headers:getHDRS(apiKey),
...
async function aiAnalyse(dayCtx, cfg, apiKey) {
  ...
  const instr = "
=== MISSION ===
Diabetologue expert. Analyse et reponds en JSON brut valide avec ces champs:
- resume: bilan 2-3 phrases
- score_equilibre: 1-10
- analyse_doses: [{repas,dose_injectee,dose_ideale,ecart,consequence_probable,lecon}]
- adaptation_ratios: {ratioIC_actuel,ratioIC_suggere,fc_actuel,fc_suggere,explication,fiabilite}
- analyse_nocturne: {bilan,glycemie_lever,qualite_lente,suggestion_lente,risque_hypo_nuit}
- recommandations: [string max 4]
- observations: [{heure,type,texte}]";
  let res;
  try {
...
function GlucidesAI({initDesc,onAccept,apiKey,photo}){
  ...
  const enhance=async()=>{
    ...
    if(!apiKey){setErr('Cle API manquante dans Mes parametres');return;}
    try{const r=photo await aiGlucidesPhoto(photo,desc,apiKey):await aiGlucides(desc,apiKey);setAiRes(r);}
    ...
    {open&&(<div style={{padding:"4px 16px 16px",borderTop:"1px solid "+C.border}}>
      ...
      <div style={{marginBottom:12}}>
        ...
        <div style={{marginTop:12,background:"#eff6ff",borderRadius:10,padding:"12px 14px"}}>
          ...
          <div style={{fontSize:11,color:C.muted}}>Pre-remplie chaque jour automatiquement.</div>
        </div>
...
function AnalysePanel({dayData,dayLabel,cfg,apiKey}){
  ...
  const enhanceAI=async()=>{
    setLoading(true);setErr(null);
    if(!apiKey){setErr('Cle API manquante dans Mes parametres');setLoading(false);return;}
...
function ExportImport({ allData, saveAll }) {
  ...
  const doExport = () => {
    ...
    a.href = url; a.download = "diabetetracker-"+toISO(new Date())+".json"; a.click();
    URL.revokeObjectURL(url);
    setMsg({ type:"ok", txt:"Donnees exportees !" });
  };
  const doImport = (file) => {
    ...
    reader.onload = e => {
      try {
        ...
        const merged = { ...allData, ...imp, days:{ ...(allData.days||{}), ...(imp.days||{}) } };
        saveAll(merged);
        setMsg({ type:"ok", txt:"Import reussi ! "+Object.keys(imp.days||{}).length+" jours fusionnes." });
      } catch(e) { setMsg({ type:"err", txt:"Fichier invalide: "+e.message }); }
      setImporting(false);
    };
    reader.readAsText(file);
  };
  return (
    <div style={{borderRadius:14,border:"1.5px solid "+C.border,background:"white",marginBottom:12,padding:"14px 16px"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
        ...
        <div><div style={{fontWeight:700,color:C.text,fontSize:15}}>Export / Import donnees</div>
          <div style={{fontSize:12,color:C.muted}}>Synchroniser PC et iPhone</div>
        </div>
      </div>
      <div style={{background:"#f0f9ff",borderRadius:8,padding:"8px 10px",marginBottom:10,fontSize:12,color:C.blue}}>
      ...
      {msg&&(<div style={{marginTop:8,padding:"8px 10px",borderRadius:8,fontSize:13,background:msg.type==="ok"?"#f0fdf4":"#fef2f2",color:msg.type==="ok"?C.green:C.red,border:"1px solid "+(msg.type==="ok"?"#86efac":"#fca5a5")}}>{msg.txt}</div>)}
    </div>
  );
}
...
function parseDexcomDate(wt) {
  // Format: "Date(1620000000000+0000)" or "/Date(1620000000000+0000)/"
  var m = String(wt).match(/Date\((\d+)/);
  if (!m) return null;
  return new Date(parseInt(m[1]));
}

function parseDexcomValue(val) {
  // Value is in mg/dL - convert to g/L
...
async function dexcomLogin(username, password, region) {
  var res = await fetch("/api/dexcom", {
    method: "POST",
  ...
  // sessionId comes back with quotes - clean it
...
function dexcomReadingsToPoints(readings) {
  // readings: [{WT, Value, Trend}, ...]
  return readings
    .map(function(r) {
      var dt = parseDexcomDate(r.WT);
      if (!dt) return null;
      var val = parseDexcomValue(r.Value);
      if (!val) return null;
      return {
...
function DexcomLive({ allData, saveAll, cfg }) {
  ...
  // Auto-refresh every 5 minutes if credentials saved
  useEffect(function() {
    if (allData.dexcomCreds && allData.dexcomCreds.sessionId) {
      doRefresh(allData.dexcomCreds);
  ...
  async function doRefresh(creds) {
    try {
      var sid = creds.sessionId;
      var readings = await dexcomReadings(sid, creds.region || "eu", 1440, 288);
      var points = dexcomReadingsToPoints(readings);
      var byDay = groupByDay(points);

      // Merge into allData.days
      var newDays = Object.assign({}, allData.days || {});
      Object.keys(byDay).forEach(function(dk) {
        newDays[dk] = Object.assign({}, newDays[dk] || {}, { dexcomCurve: byDay[dk] });
      });

      var newData = Object.assign({}, allData, { days: newDays });
      saveAll(newData);
      setLastSync(new Date());
      setStatus({ type: "ok", msg: "Synchro OK - " + points.length + " mesures" });
    } catch(e) {
      // Session expired? Try re-login
      if (e.message && (e.message.includes("401") || e.message.includes("session"))) {
        try {
          var newSid = await dexcomLogin(creds.username, creds.password, creds.region || "eu");
          var newCreds = Object.assign({}, creds, { sessionId: newSid });
          saveAll(Object.assign({}, allData, { dexcomCreds: newCreds }));
          await doRefresh(newCreds);
        } catch(e2) {
  ...
  async function connect() {
    ...
    try {
      setStatus({ type: "info", msg: "Connexion aux serveurs Dexcom..." });
      var sid = await dexcomLogin(username, password, region);
      var creds = { username, password, region, sessionId: sid };
      setStatus({ type: "info", msg: "Recuperation des donnees..." });

      var readings = await dexcomReadings(sid, region, 1440, 288);
      var points = dexcomReadingsToPoints(readings);
      var byDay = groupByDay(points);
      ...
      var newData = Object.assign({}, allData, { days: newDays, dexcomCreds: creds });
      saveAll(newData);
      setLastSync(new Date());

      var dayCount = Object.keys(byDay).length;
      setStatus({ type: "ok", msg: points.length + " mesures importees sur " + dayCount + " jours. Synchro auto toutes les 5 min." });
      setOpen(false);

      // Start auto-refresh
      if (intervalRef.current) clearInterval(intervalRef.current);
      intervalRef.current = setInterval(function() { doRefresh(creds); }, 5 * 60 * 1000);
  ...
  function disconnect() {
    if (intervalRef.current) clearInterval(intervalRef.current);
    var newData = Object.assign({}, allData);
    delete newData.dexcomCreds;
    saveAll(newData);
    setStatus(null); setLastSync(null);
    setUsername(""); setPassword("");
  }

  var isConnected = !!(allData.dexcomCreds && allData.dexcomCreds.sessionId);

  // Latest glycemia from today
  var todayCurve = allData.days && allData.days[TODAY()] && allData.days[TODAY()].dexcomCurve;
  var lastGly = todayCurve && todayCurve.length > 0 ? todayCurve[todayCurve.length - 1] : null;

  var trendArrow = { "Flat": "->", "FortyFiveUp": "/->", "SingleUp": "^", "DoubleUp": "^^", "FortyFiveDown": "\->", "SingleDown": "v", "DoubleDown": "vv", "NotComputable": "?", "RateOutOfRange": "!" };
  ...
  return (
    <div style={{ borderRadius: 14, border: "2px solid " + (isConnected ? C.green : C.blue), background: isConnected ? "#f0fdf4" : "#eff6ff", marginBottom: 12 }}>
      <div onClick={function() { setOpen(!open); }} style={{ padding: "14px 16px", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ background: isConnected ? C.green : C.blue, color: "white", borderRadius: 8, padding: "4px 10px", fontSize: 12, fontWeight: 700 }}>
            {isConnected ? "LIVE" : "Dexcom"}
          </span>
          <div>
            <div style={{ fontWeight: 700, color: C.text, fontSize: 15 }}>Dexcom Share - Temps reel</div>
            <div style={{ fontSize: 12, color: C.muted }}>
              {isConnected
                ? (lastGly ? "Derniere valeur: " + lastGly.value + " g/L " + (trendArrow[lastGly.trend] || "") + (lastSync ? " - sync " + lastSync.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" }) : "") : "Connecte - en attente de donnees")
                : "Connexion automatique a votre capteur"}
            </div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {isConnected && lastGly && (
            <span style={{ fontWeight: 800, fontSize: 18, color: glyColor(lastGly.value, cfg) }}>{lastGly.value + " g/L"}</span>
          )}
          {isConnected && (
            <button onClick={function(e) { e.stopPropagation(); doRefresh(allData.dexcomCreds); }} style={{ padding: "4px 10px", background: C.green, color: "white", border: "none", borderRadius: 6, fontSize: 11, cursor: "pointer", fontFamily: "inherit" }}>Sync</button>
          )}
          <span style={{ color: C.muted }}>{open ? "^" : "v"}</span>
        </div>
      </div>
      ...
      {open && (
        <div style={{ padding: "4px 16px 16px", borderTop: "1px solid " + (isConnected ? "#86efac" : "#bfdbfe") }}>
          {!isConnected ? (
            <div>
              <div style={{ background: "#dbeafe", borderRadius: 8, padding: "10px 12px", marginBottom: 14, fontSize: 12, color: C.blue }}>
                <strong>Pr requis :</strong> Activez le partage dans l app Dexcom :<br/>
                Dexcom app - Partager - Ajouter un suiveur (meme si pas de suiveur reel)<br/>
                Utilisez les memes identifiants que votre compte Dexcom.
              </div>
              <div style={{ marginBottom: 10 }}>
                <Lbl>Region</Lbl>
                <div style={{ display: "flex", gap: 8 }}>
                  {[["eu", "Europe (EU)"], ["us", "Etats-Unis (US)"]].map(function(item) {
                    return (
                      <button key={item[0]} onClick={function() { setRegion(item[0]); }}
                        style={{ flex: 1, padding: "8px", border: "2px solid " + (region === item[0] ? C.blue : C.border), borderRadius: 8, background: region === item[0] ? C.blue : "white", color: region === item[0] ? "white" : C.muted, cursor: "pointer", fontWeight: 700, fontSize: 12, fontFamily: "inherit" }}>
                        {item[1]}
                      </button>
                    );
                  })}
              ...
              <div style={{ marginBottom: 10 }}>
                <Lbl>Identifiant Dexcom</Lbl>
                <TInput value={username} onChange={setUsername} placeholder="Email ou nom utilisateur Dexcom" />
              </div>
              <div style={{ marginBottom: 14 }}>
                <Lbl>Mot de passe Dexcom</Lbl>
                <input type="password" value={password} onChange={function(e) { setPassword(e.target.value); }} placeholder="Mot de passe"
                  style={{ width: "100%", padding: "9px 12px", border: "1.5px solid " + C.border, borderRadius: 8, fontSize: 14, fontFamily: "inherit", boxSizing: "border-box" }} />
              </div>
              <div style={{ background: "#fef2f2", border: "1px solid #fca5a5", borderRadius: 8, padding: "8px 10px", marginBottom: 12, fontSize: 11, color: C.red }}>
                Vos identifiants sont stockes uniquement sur votre appareil. Jamais envoyes ailleurs qu a Dexcom.
              </div>
              <PBtn onClick={connect} disabled={loading || !username || !password} color={C.blue} full>
                {loading ? "Connexion..." : "Se connecter a Dexcom Share"}
              </PBtn>
            </div>
          ) : (
            <div>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <PBtn onClick={function() { doRefresh(allData.dexcomCreds); }} color={C.green} full>Synchroniser maintenant</PBtn>
